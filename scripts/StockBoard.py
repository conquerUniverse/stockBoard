...
class StockBoard:
    ...
    def getCurrBalance(
        ...
        val = sum(dfInvest[dfInvest.Description != "Dividend"].Amount) - (
            sum(dfBuy.NumberOfStocks * dfBuy.BuyingPrice)
            + sum(dfBuy.BrokerageCost)
            + sum(dfSell.BrokerageCost)

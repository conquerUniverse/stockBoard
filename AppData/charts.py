...
def getScriptsName():
    names = [
        i[:-3] for i in os.listdir(scriptPath) if i[-3:] == ".py" and i != "__init__.py"
    ]
    return names
...
strategyAvailable = [
    {"label": i.split(".")[0], "value": i.split(".")[0]} for i in getScriptsName()
]
...
strategyDropdown = dcc.Dropdown(
    id="strategy_id",
    options=strategyAvailable,
    value=None,
    style={
        "color": "black",
...
def update_figure(
    selected_value, sma_value, start_date, end_date, strategyButton, strategyName
):
    # print(sma_value)
    global df
...
def getAnalysisResult(_, strategyName, stockName):
    ...
    vw = dbc.Row(
        [
            dbc.Col(
                [
                    dbc.Row(i, className="col"),
                    dbc.Row(round(res[i], 3), className="col"),
                ],

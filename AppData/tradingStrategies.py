...
dropDownStrategies = dcc.Dropdown(id='strategy', options=[
    {'label':i,'value':i} for i in scripts_name],
...
pageHeader = dbc.CardHeader([dbc.Label("Trading Strategies",className="mr-2 mr-auto"),
    dropDownStrategies,
    dbc.Col(dbc.Button("Details",id='backtestData',className="btn btn-block "),className="h6 col-md-1 col-lg-1 col-sm-3 col-xs-6 p-0 m-0  m-xs-0"),
    dbc.Col(dbc.Button("Show Code", id="show_code",className="btn btn-block"),className="h6 col-md-1 col-lg-1 col-sm-3 col-xs-6 p-0 m-0  m-xs-0"),
    dbc.Col(dbc.Button("Run Backtest", id="runBackTest",className="btn btn-block"),className="h6 col-md-2 col-lg-2 col-sm-3 col-xs-6 p-0 m-0  m-xs-0")],
    className="row"
    )

alertView = dbc.Alert(id="alert",className="row",color='success',is_open=False, dismissable=True)
...
layout = dbc.Container([
    dbc.Card([pageHeader,alertView,pageBody],outline=True,className="w-100 p-0 m-0"),

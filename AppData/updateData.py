...
stockNameDropdown = dcc.Dropdown(
    id="name",
    options=stockNames,
    value=None,
        ...
        dbc.CardHeader(
            [
                ...
                dbc.Button(
                    "Download Data ",
                    id="download_data",
                    className="float-md-right btn col-2 btn-md ",
                    outline=True,
                ),
            ],
            className="row",
        ),
        dbc.Alert(id="messageSave", className="row", dismissable=True, color="success"),
...
def addStructuredData(category):
    nameAndDate = dbc.FormGroup(
        [
            dbc.Row(
                [dbc.Label("Name", className="mr-2", width=4), stockNameDropdown],
                style={"width": "50%"},
            ),
            dbc.Row(
                [
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Input(
                        type="number", placeholder="No. of stock", id="numberofstocks"
                    ),
                ],
                style={"width": "50%"},
            ),
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Input(
                        type="number", placeholder="Cost of transaction", id="totalcost"
                    ),
                ],
                style={"width": "50%"},
            ),
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Input(
                        type="number", placeholder="amount added ", id="amountInvest"
                    ),
                ],
                style={"width": "50%"},
            ),
    ...
    submitButton = dbc.Row(
        [
            dbc.Button(
                f"{category.capitalize()} Stock",
                color="primary",
                n_clicks=0,
                id="submit",
            )
    ...
    buySellForm = html.Div(
        dbc.FormGroup([nameAndDate, numberAndPriceOfStock, totalCostAndExtraCharge]),
        id="buySellForm",
    )
    investForm = html.Div(
        dbc.FormGroup(
    ...
    return dbc.Form(
        [
            buySellForm,
            investForm,
            dbc.FormGroup([password, submitButton]),
            html.Hr(),
            saveData,
        ],
        inline=True,
    ...
    if ischanged(args[0]):
        if isPasswordMatching(args[-2]):  # submit button is pressed
            ...
            if putData(args[-1], *args[1:-2]):
                return (
                    args[-1] + " Data is updated",
                    formToggle(args[-1]),
                    not formToggle(args[-1]),
                )
            else:
                return (
...
def render_tab_content(active_tab, data=1):
    ...
    if active_tab and data is not None:
        ...
        form = dbc.Card(
            [
                dbc.CardHeader(
                    "Update " + str(active_tab) + " Data", className="display-5"
                ),
                dbc.CardBody(addStructuredData(active_tab)),
                dbc.CardFooter(message),
            ],
            outline=True,
        )
...
@app.callback(Output("download", "data"), Input("download_data", "n_clicks"))
